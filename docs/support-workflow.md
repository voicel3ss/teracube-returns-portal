...
- Anyone with Support access can open an unassigned case and explicitly claim it. Claiming assigns the case to that person. The claim is concurrency-safe, so two staff members cannot silently take the same case.
...
- An Admin can claim an unassigned case and then assign that claimed case to a different active staff member with Support access. Self-assignment is done only through **Claim work**. Assignments are audited; staff cannot take work already assigned to someone else.
- Search accepts replacement order number, device serial, or customer email. Results, customer records, and order details show every associated email in full to authorized Support, Oversight, and Admin users; Repair and Logistics never receive parent email data.
