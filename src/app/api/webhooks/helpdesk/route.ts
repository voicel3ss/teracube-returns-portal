...
export async function POST(request: Request) {
  ...
  const order = await prisma.replacementOrder.findFirst({ where: { communicationTicketId: parsed.data.ticketId }, include: { workItems: { where: { team: "support", status: { not: "completed" } }, select: { id: true, pauseReason: true } } } }); if (!order) return Response.json({ error: "Ticket is not linked to an order." }, { status: 404 });
  try {
    ...
    await tx.workItem.updateMany({
      where: { replacementOrderId: order.id, team: "support", assignedToStaffId: { not: null }, OR: [{ status: "claimed" }, { status: "snoozed", pauseReason: { in: customerReplyResumablePauseReasons } }] },
      data: { status: "claimed", lastActivityAt: new Date(), snoozedUntil: null, pauseReason: null },
    });
    await tx.workItem.updateMany({
      where: { replacementOrderId: order.id, team: "support", status: "open", assignedToStaffId: null },
    ...
    if (order.reviewState === "needs_clarification") {
      ...
      await tx.workItem.upsert({
        ...
        update: { snoozedUntil: null, pauseReason: null, lastActivityAt: new Date() },
        create: { replacementOrderId: order.id, team: "support", kind: "needs_clarification", status: "open" },
      });
    } else if (order.workItems.length > 0) {
      await tx.workItem.updateMany({ where: { replacementOrderId: order.id, team: "support", status: { in: ["open", "claimed"] } }, data: { lastActivityAt: new Date(), snoozedUntil: null, pauseReason: null } });
    } else {
      await tx.workItem.upsert({
