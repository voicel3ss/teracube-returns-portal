...
export async function POST(request: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (shipment.replacementOrderId && shipment.replacementOrder) {
      ...
      if (shipment.type === "inbound" && status === "in_transit") orderStatus = "return_in_transit";
      if (shipment.type === "outbound" && status === "delivered") orderStatus = "refurb_delivered";
      const inboundDone = shipment.type === "inbound" ? status === "delivered" : shipment.replacementOrder.shipments.some((item) => item.type === "inbound" && ["delivered", "received"].includes(item.status));
      ...
      if (inboundDone && outboundDone) orderStatus = "closed";
