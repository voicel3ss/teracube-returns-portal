...
export async function POST(request: Request) {
  ...
  const terminalOrder = ["closed", "return_discrepancy", "unidentified"].includes(order.status);
  const alreadyBlocked = order.status === "fulfillment_blocked";
  try {
    await prisma.$transaction(async (tx) => {
      ...
      else if (!terminalOrder && !alreadyBlocked) { await tx.replacementOrder.update({ where: { id: order.id }, data: { status: "fulfillment_blocked", resolution: "exception" } }); await tx.workItem.upsert({ where: { replacementOrderId_kind: { replacementOrderId: order.id, kind: "fulfillment_blocked" } }, update: { status: "open", assignedToStaffId: null, snoozedUntil: null, pauseReason: null, lastActivityAt: new Date() }, create: { replacementOrderId: order.id, team: "support", kind: "fulfillment_blocked" } }); await tx.conversationMessage.create({ data: { replacementOrderId: order.id, senderKind: "system", body: "Your replacement is delayed while our team sources the correct device. We’ll update you here." } }); }
      await tx.auditEvent.create({ data: { actorKind: "provider", action: parsed.data.type, entityType: "replacement_order", entityId: order.id, metadata: { providerEventId: parsed.data.id, ignoredTerminalOrder: parsed.data.type === "fulfillment.blocked" && terminalOrder, ignoredExistingBlock: parsed.data.type === "fulfillment.blocked" && alreadyBlocked } } });
    });
  } catch (error) {
