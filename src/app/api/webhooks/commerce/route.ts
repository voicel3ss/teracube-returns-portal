...
export async function POST(request: Request) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      else if (!terminalOrder && !alreadyBlocked) { await tx.replacementOrder.update({ where: { id: order.id }, data: { status: "fulfillment_blocked", resolution: "exception" } }); await tx.workItem.upsert({ where: { replacementOrderId_kind: { replacementOrderId: order.id, kind: "fulfillment_blocked" } }, update: { status: "open", assignedToStaffId: null, snoozedUntil: null, lastActivityAt: new Date() }, create: { replacementOrderId: order.id, team: "support", kind: "fulfillment_blocked" } }); await tx.conversationMessage.create({ data: { replacementOrderId: order.id, senderKind: "system", body: "Your replacement is delayed while our team sources the correct device. We’ll update you here." } }); }
