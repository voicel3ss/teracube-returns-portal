...
export default async function SupportQueuePage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  const [items, searchResults, overdueOrderIds] = await Promise.all([
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-7 sm:py-10">
        ...
        <div className="mt-8 grid gap-7 lg:grid-cols-[1fr_0.55fr]">
          <section>
            ...
            <QueueList items={myItems} overdueOrderIds={overdueOrderIds} empty="Open a case from the team queue to start working." />
          </section>
          <section>
            ...
            <QueueList items={teamItems} overdueOrderIds={overdueOrderIds} empty="The support queue is clear." />
          </section>
        </div>
      </div>
    </StaffShell>
...
function QueueList({ items, overdueOrderIds, empty }: { items: QueueListItem[]; overdueOrderIds: Set<string>; empty: string }) {
