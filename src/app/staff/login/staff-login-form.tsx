...
export function StaffLoginForm({ googleClientId }: { googleClientId?: string }) {
  ...
  async function requestCode() {
    ...
    try {
      ...
      setLocalCode(body.verificationCode ?? null);
    } catch (caught) {
    ...
    <form onSubmit={verifyCode} className={googleClientId ? "" : "mt-7"}>
      ...
      {challengeId ? (
        <>
          ...
          />
          {localCode ? <p className="mt-3 rounded-xl bg-[var(--mint)]/25 px-4 py-3 text-xs text-black/55">Local verification code: <strong className="font-mono text-black">{localCode}</strong></p> : null}
